...
bool TaskDebugBranchCheck::hasAnnotation(Instruction *i, Value *V,
                                         StringRef Ann, uint8_t level) {
...
bool TaskDebugBranchCheck::successor_visited(
    BasicBlock *blk, std::map<BasicBlock *, int> &bbVisitedMap) {
...
void TaskDebugBranchCheck::instrument_access(Instruction *inst, Value *addr,
                                             Constant *rdWr,
  ...
  BitCastInst *bitcast = new BitCastInst(
      addr, PointerType::getUnqual(Type::getInt8Ty(inst->getContext())), "",
      inst);
  ...
  if (locks_acq.size() == 0) {
    ConstantPointerNull *null_ptr = ConstantPointerNull::get(
        PointerType::getUnqual(Type::getInt64Ty(inst->getContext())));
    args.push_back(null_ptr);
  } else {
    AllocaInst *lock_aq_alloca = new AllocaInst(
        ArrayType::get(Type::getInt64Ty(inst->getContext()), locks_acq.size()),
        "", inst);
    ...
    for (std::vector<Value *>::iterator it = locks_acq.begin();
         it != locks_acq.end(); ++it, ++j) {
      SmallVector<Value *, 8> idx;
      idx.push_back(base_idx);
      idx.push_back(ConstantInt::get(Type::getInt32Ty(inst->getContext()), j));
      GetElementPtrInst *lock_elem = GetElementPtrInst::CreateInBounds(
          ArrayType::get(Type::getInt64Ty(inst->getContext()),
                         locks_acq.size()),
          lock_aq_alloca, idx, "", inst);
  ...
  if (locks_rel.size() == 0) {
    ...
    AllocaInst *lock_rel_alloca = new AllocaInst(
        ArrayType::get(Type::getInt64Ty(inst->getContext()), locks_rel.size()),
        "", inst);
    ...
    Constant *base_idx =
        ConstantInt::get(Type::getInt32Ty(inst->getContext()), 0);
    for (std::vector<Value *>::iterator it = locks_rel.begin();
         it != locks_rel.end(); ++it, ++j) {
      SmallVector<Value *, 8> idx;
      idx.push_back(base_idx);
      idx.push_back(ConstantInt::get(Type::getInt32Ty(inst->getContext()), j));
      GetElementPtrInst *lock_elem = GetElementPtrInst::CreateInBounds(
...
void TaskDebugBranchCheck::addFunctionSummaries(BasicBlock *from_bb,
                                                BasicBlock *to_bb,
  ...
  for (BasicBlock::iterator i = from_bb->begin(); i != from_bb->end(); ++i) {
    ...
    case Instruction::Load: {
      ...
      if (hasAnnotation(i, op_l, "check_av", 1)) {
        ...
        instrument_access(to_bb->getFirstNonPHI(), op_l, read, locks_acq,
                          locks_rel);
      }
      break;
    }
    case Instruction::Store: {
      ...
      if (hasAnnotation(i, op_s, "check_av", 1)) {
        ...
        instrument_access(to_bb->getFirstNonPHI(), op_s, write, locks_acq,
                          locks_rel);
      }
      break;
    }
...
void TaskDebugBranchCheck::checkForBranch(TerminatorInst *TInst) {
  ...
  if (TInst_Succ_1->getNumSuccessors() == 0 &&
      TInst_Succ_2->getNumSuccessors() == 0) {
...
void TaskDebugBranchCheck::parseBB(BasicBlock &blk,
...
void TaskDebugBranchCheck::intializeBranchCheckVariables(Module &module) {
  ...
  assert(recordAccess && "RecordAccess function type null?");

  lockAcquire = module.getFunction("CaptureLockAcquire");
  assert(lockAcquire && "CaptureLockAcquire function type null?");

  lockRelease = module.getFunction("CaptureLockRelease");
  assert(lockRelease && "CaptureLockRelease function type null?");
}
...
void TaskDebugBranchCheck::intializeBranchCheckFunctions(Module &module) {
  ...
  module.getOrInsertFunction("RecordAccess", VoidTy, size_ty, void_ptr_ty,
                             uint_ptr_ty, size_ty, uint_ptr_ty, size_ty, int_ty,
                             NULL);
  module.getOrInsertFunction("CaptureLockAcquire", VoidTy, size_ty, size_ty,
                             NULL);
  module.getOrInsertFunction("CaptureLockRelease", VoidTy, size_ty, size_ty,
                             NULL);
}
...
static RegisterPass<TaskDebugBranchCheck> X("task_debug_branch_check",
                                            "TaskDebugBranchCheck Pass");
