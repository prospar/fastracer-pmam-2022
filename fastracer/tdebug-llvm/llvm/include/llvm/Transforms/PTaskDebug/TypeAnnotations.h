...
template <typename ImplClass> class Annotator : public StmtVisitor<ImplClass> {
public:
  CompilerInstance &CI;
  ImplClass *impl;
  ...
  Annotator(CompilerInstance &_ci, bool _instrument)
      : CI(_ci), impl(static_cast<ImplClass *>(this)), CurFunc(NULL),
  ...
  bool Compatible(QualType LTy, QualType RTy) const { return true; }
  ...
  DiagnosticsEngine &Diags() const { return CI.getDiagnostics(); }

  void EmitIncompatibleError(clang::Stmt *S, QualType LTy, QualType RTy) {
    unsigned did = Diags().getCustomDiagID(DiagnosticsEngine::Error,
  ...
  void VisitCallExpr(CallExpr *E) { visitCall(E); }
  void VisitCXXMemberCallExpr(CallExpr *E) { visitCall(E); }
  void VisitCXXOperatorCallExpr(CallExpr *E) { visitCall(E); }
...
template <typename AnnotatorClass> class TAConsumer : public ASTConsumer {
public:
  CompilerInstance &CI;
  ...
  TAConsumer(CompilerInstance &_ci, bool _instrument)
      : CI(_ci), Annotator(_ci, _instrument), Instrument(_instrument) {}
  ...
  virtual void Initialize(ASTContext &Context) {
    ...
    if (Instrument) {
      ...
      auto it = std::find(hmxc->Consumers.begin(), hmxc->Consumers.end(), this);
