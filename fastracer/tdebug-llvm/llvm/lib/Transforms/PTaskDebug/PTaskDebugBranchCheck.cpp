#include "llvm/Transforms/PTaskDebug/PTaskDebugBranchCheckPass.h"

#include<vector>
#include<stack>

PTaskDebugBranchCheck::PTaskDebugBranchCheck():ModulePass(ID) {
}

bool PTaskDebugBranchCheck::hasAnnotation(Instruction* i, Value *V, StringRef Ann, uint8_t level) {
...
bool PTaskDebugBranchCheck::instru_fn(StringRef fn_name) {
...
bool PTaskDebugBranchCheck::successor_visited(BasicBlock* blk, std::map<BasicBlock*, int>& bbVisitedMap) {
...
void PTaskDebugBranchCheck::instrument_access(Instruction* inst, 
					     Value* addr,
					     Constant* rdWr, 
...
void PTaskDebugBranchCheck::addFunctionSummaries(BasicBlock* from_bb, BasicBlock* to_bb, Instruction* first_inst) {
...
void PTaskDebugBranchCheck::checkForBranch(TerminatorInst *TInst) {
...
void PTaskDebugBranchCheck::parseBB(BasicBlock& blk, std::map<BasicBlock*, int>& bbVisitedMap) {
...
void PTaskDebugBranchCheck::intializeBranchCheckVariables(Module& module) {
...
void PTaskDebugBranchCheck::intializeBranchCheckFunctions(Module& module) {
...
bool PTaskDebugBranchCheck::runOnModule(Module &module) {
#if 1
...
char PTaskDebugBranchCheck::ID = 0;
static RegisterPass<PTaskDebugBranchCheck> X("ptask_debug_branch_check", "Ptracer TaskDebugBranchCheck Pass");
