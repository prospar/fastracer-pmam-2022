...
//    set_complete();
    ...
    for (b = begin; b != end; b++) {
      for (j = 1; j <= iN - 1; ++j) {
        ...
        RecordMem(thdId, &(randZ[l][BLOCKSIZE * j + b]), READ);
        // RecordMem(thdId, &j, READ);
        // RecordMem(thdId, &b, READ);
        // RecordMem(thdId, &b, READ);
        pdZ[l][BLOCKSIZE * j + b] =
            CumNormalInv(randZ[l][BLOCKSIZE * j + b]); /* 18% of the total executition time */
    ...
    for (int s = 0; s < 1; s++) {
      for (j = 1; j <= iN - 1; ++j) {
        for (l = 0; l <= iFactors - 1; ++l) {
          ...
          randZ[l][BLOCKSIZE * j + b + s] =
              RanUnif(lRndSeed); /* 10% of the total executition time */
        }
      }
    }
  }
...
//  ParallelB B1(pdZ, randZ, BLOCKSIZE, iN);
  for (l = 0; l <= iFactors - 1; ++l) {
    B.set_l(l);
//  std::cout << "parallel_for 2 start\n";
    tbb::parallel_for(tbb::blocked_range<int>(0, BLOCKSIZE, PARALLEL_B_GRAINSIZE), B);
//  std::cout << "parallel_for 2 end\n";
  }
  ...
  for (int b = 0; b < BLOCKSIZE; b++) { // b is the blocks
    for (j = 1; j <= iN - 1; ++j) {     // j is the timestep
      ...
      for (l = 0; l <= iN - (j + 1); ++l) { // l is the future steps
        ...
        ppdHJMPath[j][BLOCKSIZE * l + b] = ppdHJMPath[j - 1][BLOCKSIZE * (l + 1) + b] +
