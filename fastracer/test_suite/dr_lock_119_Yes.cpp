...
public:
  task* execute() {
    ...
    my_getlock(&y);
    CaptureLockAcquire(get_cur_tid(), (ADDRINT)&y);

    // READ & WRITE
    shd[getTaskId()]++;

    CaptureLockRelease(get_cur_tid(), (ADDRINT)&y);
    my_releaselock(&y);
    //myLock.release();
...
public:
  task* execute() {
    ...
    my_getlock(&x);
    CaptureLockAcquire(get_cur_tid(), (ADDRINT)&x);

    // READ & WRITE
    shd[2]++;

    CaptureLockRelease(get_cur_tid(), (ADDRINT)&x);
    my_releaselock(&x);
    // myLock.release();
    __exec_end__(getTaskId());
    return NULL;
