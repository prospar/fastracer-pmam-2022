...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  Opts.setInlining((Opts.OptimizationLevel > 1)
  ...
  Opts.UseRegisterSizedBitfieldAccess =
      Args.hasArg(OPT_fuse_register_sized_bitfield_access);
  ...
  Opts.PTaskDebug = Args.hasArg(OPT_fptaskdebug);
  Opts.Prospar = Args.hasArg(OPT_fprospar); // PROSPAR: Added
  Opts.SSPBufferSize =
...
static InputKind ParseFrontendArgs(FrontendOptions &Opts, ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(OPT_Action_Group)) {
    ...
    case OPT_emit_pch:
      ...
      break;
    case OPT_emit_pth:
      Opts.ProgramAction = frontend::GeneratePTH;
      break;
    case OPT_init_only:
  ...
  Opts.CodeCompleteOpts.IncludeMacros = Args.hasArg(OPT_code_completion_macros);
  ...
  if (const Arg *A = Args.getLastArg(OPT_arcmt_check, OPT_arcmt_modify,
                                     OPT_arcmt_migrate)) {
    switch (A->getOption().getID()) {
    default:
  ...
  Opts.ObjCMTWhiteListPath =
      Args.getLastArgValue(OPT_objcmt_whitelist_dir_path);
...
#include "clang/Frontend/LangStandards.def"
  ...
  if (Args.hasArg(OPT_ftrapv)) {
    ...
    Opts.OverflowHandler = Args.getLastArgValue(OPT_ftrapv_handler);
  ...
  Opts.MaxTypeAlign =
      getLastArgIntValue(Args, OPT_fmax_type_align_EQ, 0, Diags);
  ...
  if (Arg *A = Args.getLastArg(OPT_fms_memptr_rep_EQ)) {
    LangOptions::PragmaMSPointersToMembersKind InheritanceModel =
        llvm::StringSwitch<LangOptions::PragmaMSPointersToMembersKind>(
            A->getValue())
            .Case("single", LangOptions::PPTMK_FullGeneralitySingleInheritance)
            .Case("multiple",
  ...
  case 0:
    ...
    break;
  case 1:
    Opts.setStackProtector(LangOptions::SSPOn);
    break;
  case 2:
    Opts.setStackProtector(LangOptions::SSPStrong);
    break;
  case 3:
    Opts.setStackProtector(LangOptions::SSPReq);
    break;
  }
...
void ModuleSignature::add(StringRef Value) {
  for (StringRef::iterator I = Value.begin(), IEnd = Value.end(); I != IEnd;
       ++I)
    add(*I, 8);
}
...
#include "clang/Basic/LangOptions.def"
  ...
  code =
      hash_combine(code, TargetOpts->Triple, TargetOpts->CPU, TargetOpts->ABI);
