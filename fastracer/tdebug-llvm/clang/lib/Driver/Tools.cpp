...
static void addPGOAndCoverageFlags(Compilation &C, const Driver &D,
  ...
  else if (ProfileGenerateArg && ProfileGenerateArg->getOption().matches(
                                     options::OPT_fprofile_generate_EQ)) {
...
// Disable the verification pass in -asserts builds.
  ...
  if (Args.getLastArg(options::OPT_fptaskdebug)) {
    CmdArgs.push_back("-fptaskdebug");
  }

  // PROSPAR: Added
  if (Args.getLastArg(options::OPT_fprospar)) {
    CmdArgs.push_back("-fprospar");
  }
...
static std::string getLinuxDynamicLinker(const ArgList &Args,
  ...
  if (ToolChain.getTriple().getEnvironment() == llvm::Triple::Android) {
    ...
    if (ToolChain.getTriple().getEnvironment() == llvm::Triple::GNUEABIHF ||
        tools::arm::getARMFloatABI(ToolChain.getDriver(), Args,
                                   ToolChain.getTriple()) == "hard")
