...
public:
  ...
  void operator()(const tbb::blocked_range<int> &range, size_t thdid) {
    double myhiz = 0;
    // RecordMem(thdid, &points->dim, READ);
...
#if LINE_NO_PASS
      RecordMem(thdid, &points->p[kk].weight, READ, 114);
      RecordMem(thdid, &points->p[0], READ, 115);
      RecordMem(thdid, &points->p[kk], READ, 116);
#else
      RecordMem(thdid, &points->p[kk].weight, READ);
      RecordMem(thdid, &points->p[0], READ);
      RecordMem(thdid, &points->p[kk], READ);
#endif
      myhiz += dist(points->p[kk], points->p[0], ptDimension) * points->p[kk].weight;
    }
    // RecordMem(thdid, &hiz, READ);
    // RecordMem(thdid, &hiz, WRITE);
    hiz += myhiz;
  }
...
#if LINE_NO_PASS
      RecordMem(thdid, &points->p[k], READ, 142);
      RecordMem(thdid, &points->p[0], READ, 143);
      RecordMem(thdid, &points->dim, READ, 144);
#else
      RecordMem(thdid, &points->p[(k+8)%(points->num)], WRITE);
      RecordMem(thdid, &points->p[0], READ);
      RecordMem(thdid, &points->dim, READ);
#endif
      float distance = dist(points->p[k], points->p[0], points->dim);
#if LINE_NO_PASS
      RecordMem(thdid, &points->p[k].weight, READ, 152);
#else
...
public:
  ...
  void operator()(const tbb::blocked_range<int> &range, size_t thdid) {
    ...
    if (type) {
      ...
      for (int k = begin; k != end; k++) {
#if LINE_NO_PASS
        RecordMem(thdid, &points->p[k].cost, READ, 188);
#else
        RecordMem(thdid, &points->p[k].cost, READ);
#endif
        local_total += points->p[k].cost;
      }
      // RecordMem(thdid, &total_cost, READ);
...
#if LINE_NO_PASS
        RecordMem(thdid, &points->p[k].weight, READ, 201);
        RecordMem(thdid, &points->p[k].cost, READ, 202);
#else
        RecordMem(thdid, &points->p[k].weight, READ);
        RecordMem(thdid, &points->p[k].cost, READ);
#endif
...
#if LINE_NO_PASS
          RecordMem(thdid, &points->p[k].weight, READ, 209);
#else
          RecordMem(thdid, &points->p[k].weight, READ);
#endif
          // RecordMem(thdid, &points->p[k].cost, WRITE);
...
public:
  ...
  task *execute() {
    ...
    for (int i = k1; i < k2; i++) {
      if (is_center[i]) {
#if LINE_NO_PASS
        RecordMem(get_cur_tid(), &center_table[i], WRITE, 254);
#else
...
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &work_mem[p * stride], READ, 302);
#else
...
public:
  ...
  task *execute() {
...
#if LINE_NO_PASS
    RecordMem(get_cur_tid(), &points->num, READ, 407);
#else
    ...
    if (pid == (NUM_DIVISIONS)-1) {
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &points->num, READ, 418);
#else
      RecordMem(get_cur_tid(), &points->num, READ);
#endif
      k2 = points->num;
...
public:
  ...
  task *execute() {
...
#if LINE_NO_PASS
    RecordMem(get_cur_tid(), &points->num, READ, 524);
#else
    RecordMem(get_cur_tid(), &points->num, READ);
#endif
    long bsize = points->num / ((NUM_DIVISIONS)); //
    ...
    if (pid == (NUM_DIVISIONS)-1) {
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &points->num, READ, 534);
#else
      RecordMem(get_cur_tid(), &points->num, READ);
#endif
      k2 = points->num;
...
public:
  ...
  task *execute() {
    ...
    double *gl_lower = &work_mem[(NUM_DIVISIONS)*stride];
#if LINE_NO_PASS
    RecordMem(get_cur_tid(), &points->num, READ, 623);
#else
    RecordMem(get_cur_tid(), &points->num, READ);
#endif
    long bsize = points->num / ((NUM_DIVISIONS)); // points->num/1;//((NUM_DIVISIONS));
    ...
    if (pid == (NUM_DIVISIONS)-1) {
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &points->num, READ, 634);
#else
      RecordMem(get_cur_tid(), &points->num, READ);
#endif
      k2 = points->num;
    ...
    for (int i = k1; i < k2; i++) {
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &points->p[i].assign, READ,647);
      RecordMem(get_cur_tid(), &center_table[points->p[i].assign], READ,648);
      RecordMem(get_cur_tid(), &gl_lower[center_table[points->p[i].assign]], READ,649);
#else
      RecordMem(get_cur_tid(), &points->p[i].assign, READ);
...
#if LINE_NO_PASS
        RecordMem(get_cur_tid(), &points->p[i].weight, READ, 660);
        RecordMem(get_cur_tid(), &points->p[i].cost, WRITE, 661);
#else
...
#if LINE_NO_PASS
        RecordMem(get_cur_tid(), &points->p[i].assign, READ, 668);
#else
        RecordMem(get_cur_tid(), &points->p[i].assign, READ);
#endif
        points->p[i].assign = x;
    ...
    for (int i = k1; i < k2; i++) {
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &is_center[i], READ, 678);
      RecordMem(get_cur_tid(), &center_table[i], READ, 679);
      RecordMem(get_cur_tid(), &gl_lower[center_table[i]], READ, 680);
#else
...
#if LINE_NO_PASS
        RecordMem(get_cur_tid(), &is_center[i], READ, 688);
#else
        RecordMem(get_cur_tid(), &is_center[i], READ);
#endif
        is_center[i] = false;
...
#if LINE_NO_PASS
      RecordMem(get_cur_tid(), &is_center[i], READ, 694);
#else
      RecordMem(get_cur_tid(), &is_center[i], READ);
#endif
      is_center[x] = true;
  ...
  */
  ...
  /*****  loopA() *****/
  {
    CenterTableCountTask &t =
        *new (tbb::task::allocate_root()) CenterTableCountTask(stride, points, work_mem);
    tbb::t_debug_task::spawn_root_and_wait(t);
  }
  ...
  {
    LowerCostTask &t =
        *new (tbb::task::allocate_root()) LowerCostTask(stride, points, x, work_mem, K);
    tbb::t_debug_task::spawn_root_and_wait(t);
  }
  ...
  {
    CenterCloseTask &t =
        *new (tbb::task::allocate_root()) CenterCloseTask(stride, points, work_mem, K, z);
    tbb::t_debug_task::spawn_root_and_wait(t);
  }
...
#ifdef TBB_VERSION
float pFL(Points *points, int *feasible, int numfeasible, double z, long *k, double cost, long iter,
          double e) {
...
#else //! TBB_VERSION
float pFL(Points *points, int *feasible, int numfeasible, float z, long *k, double cost, long iter,
          float e, int pid, pthread_barrier_t *barrier) {
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
  long i;
...
float pkmedian(Points *points, long kmin, long kmax, long *kfinal, int pid,
  ...
  while (1) {
    ...
    lastcost = cost;
    cost = pFL(points, feasible, numfeasible, z, &k, cost, (long)(ITER * kmax * log((double)kmax)),
               0.1);
...
void streamCluster(PStream *stream, long kmin, long kmax, int dim, long chunksize, long centersize,
                   char *outfile) {
