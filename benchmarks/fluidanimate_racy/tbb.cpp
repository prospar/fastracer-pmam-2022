...
int *cnumPars = 0;
int *cnumPars2 = 0;
Cell **last_cells = NULL; // helper array with pointers to last cell structure of "cells" array
                          // lists
#ifdef ENABLE_VISUALIZATION
...
void InitSim(char const *fileName, unsigned int threadnum) {
  ...
  for (int i = 0; i < NUM_GRIDS * NUM_TASKS; i++)
    cellpool_init(&pools[i], numParticles / (NUM_GRIDS * NUM_TASKS));
  ...
  fptype particleMass = 0.5 * doubleRestDensity /
                        (restParticlesPerMeter * restParticlesPerMeter * restParticlesPerMeter);
          ...
          for (int dk = -1; dk <= 1; ++dk)
            for (int dj = -1; dj <= 1; ++dj)
              for (int di = -1; di <= 1; ++di) {
                ...
                if (ci < grids[i].sx || ci >= grids[i].ex || cj < grids[i].sy ||
                    cj >= grids[i].ey || ck < grids[i].sz || ck >= grids[i].ez)
                  border[index] = true;
              }
        }
  ...
  assert((cells != NULL) && (cells2 != NULL) && (cnumPars != NULL) && (cnumPars2 != NULL) &&
         (last_cells != NULL));
  ...
  int rv4 =
      posix_memalign((void **)(&last_cells), CACHELINE_SIZE, sizeof(struct Cell *) * numCells);
...
template <class T> class WorkerLauncher : public tbb::t_debug_task {
...
public:
  ...
  task *execute() {
    ...
    // AV_Detector::RecordMem(get_cur_tid(), &(grids[i_].sz), READ);
    sz = grids[i_].sz;
    // AV_Detector::RecordMem(get_cur_tid(), &(grids[i_].sz), READ);
    ez = grids[i_].ez;
...
template <class T> class GridLauncher : public tbb::t_debug_task {
  bool is_continuation;
  int num_grids;
...
public:
  ClearParticlesMTWorker(int id, int tid_, int s, int e) : pid(id), tid(tid_), start(s), end(e) {}

  task *execute() {
    __exec_begin__(getTaskId());

    // AV_Detector::RecordMem(get_cur_tid(), &end, READ);
    // AV_Detector::RecordMem(get_cur_tid(), &start, READ);
    int block = (end - start) / NUM_TASKS;
    // AV_Detector::RecordMem(get_cur_tid(), &start, READ);
    // AV_Detector::RecordMem(get_cur_tid(), &pid, READ);
    ...
    for (int iz = sz; iz < ez; ++iz) {
      // AV_Detector::RecordMem(get_cur_tid(), &(grids[tid].ey), READ);
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        // AV_Detector::RecordMem(get_cur_tid(), &(grids[tid].ex), READ);
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          // AV_Detector::RecordMem(get_cur_tid(), &nx, READ);
          int index = (iz * ny + iy) * nx + ix;
#if LINE_NO_PASS
          RecordMem(get_cur_tid(), &cnumPars[index], WRITE, 586);
#else
          RecordMem(get_cur_tid(), &cnumPars[index%20], WRITE);
#endif
//          shivam++;
          cnumPars[index] = 0;
          // AV_Detector::RecordMem(get_cur_tid(), &cells[index], WRITE);
          cells[index].next = NULL;
          // AV_Detector::RecordMem(get_cur_tid(), &last_cells[index], WRITE);
...
public:
  RebuildGridMTWorker(int id, int tid_, int s, int e) : pid(id), tid(tid_), start(s), end(e) {}
  ...
  task *execute() {
    ...
    for (int iz = sz; iz < ez; ++iz) {
      // AV_Detector::RecordMem(get_cur_tid(), &grids[tid].sy, READ);
              if(getTaskId() == 695){
                std::cout << "TaskId 695\n";
              }
      // AV_Detector::RecordMem(get_cur_tid(), &grids[tid].ey, READ);
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        // AV_Detector::RecordMem(get_cur_tid(), &grids[tid].sx, READ);
        // AV_Detector::RecordMem(get_cur_tid(), &grids[tid].ex, READ);
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          int index2 = (iz * ny + iy) * nx + ix;
          Cell *cell2 = &cells2[index2];
#if LINE_NO_PASS
          RecordMem(get_cur_tid(), &cnumPars2[index2], READ, 634);
#else
          RecordMem(get_cur_tid(), &cnumPars2[index2], READ);
#endif
          int np2 = cnumPars2[index2];
          // iterate through source particles
          for (int j = 0; j < np2; ++j) {
            // get destination for source particle
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->p[j % PARTICLES_PER_CELL].x), READ);
            // AV_Detector::RecordMem(get_cur_tid(), &(delta.x), READ);
            int ci = (int)((cell2->p[j % PARTICLES_PER_CELL].x - domainMin.x) / delta.x);
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->p[j % PARTICLES_PER_CELL].y), READ);
            // AV_Detector::RecordMem(get_cur_tid(), &(delta.y), READ);
            int cj = (int)((cell2->p[j % PARTICLES_PER_CELL].y - domainMin.y) / delta.y);
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->p[j % PARTICLES_PER_CELL].z), READ);
            // AV_Detector::RecordMem(get_cur_tid(), &(delta.z), READ);
            ...
            if (!cfl_cond_satisfied) {
              std::cerr << "FATAL ERROR: Courant–Friedrichs–Lewy condition not satisfied."
                        << std::endl;
              exit(1);
            }
...
#if LINE_NO_PASS
              RecordMem(get_cur_tid(), &(last_cells[index]), READ, 692);
#else
              RecordMem(get_cur_tid(), &(last_cells[index]), READ);
#endif
              cell = last_cells[index];
#if LINE_NO_PASS
              RecordMem(get_cur_tid(), &(cnumPars[index]), READ, 694);
#else
              RecordMem(get_cur_tid(), &(cnumPars[index]), READ);
#endif
              np = cnumPars[index];

              // add another cell structure if everything full
#if LINE_NO_PASS
              RecordMem(get_cur_tid(), &(cnumPars[index]), READ, 698);
#else
              RecordMem(get_cur_tid(), &(cnumPars[index]), READ);
#endif
              if ((np % PARTICLES_PER_CELL == 0) && (cnumPars[index] != 0)) {
                ...
                cell = cell->next;
#if LINE_NO_PASS
                RecordMem(get_cur_tid(), &(last_cells[index]), WRITE, 702);
#else
                RecordMem(get_cur_tid(), &(last_cells[index]), WRITE);
#endif
            ...
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->hv[j % PARTICLES_PER_CELL].x), READ);
            cell->hv[np % PARTICLES_PER_CELL].x = cell2->hv[j % PARTICLES_PER_CELL].x;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->hv[j % PARTICLES_PER_CELL].y), READ);
            cell->hv[np % PARTICLES_PER_CELL].y = cell2->hv[j % PARTICLES_PER_CELL].y;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->hv[j % PARTICLES_PER_CELL].z), READ);
            cell->hv[np % PARTICLES_PER_CELL].z = cell2->hv[j % PARTICLES_PER_CELL].z;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->v[j % PARTICLES_PER_CELL].x), READ);
            cell->v[np % PARTICLES_PER_CELL].x = cell2->v[j % PARTICLES_PER_CELL].x;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->v[j % PARTICLES_PER_CELL].y), READ);
            cell->v[np % PARTICLES_PER_CELL].y = cell2->v[j % PARTICLES_PER_CELL].y;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell2->v[j % PARTICLES_PER_CELL].z), READ);
    ...
    for (int iz = sz; iz < ez; ++iz) {
      ...
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        ...
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          ...
          Cell *cell = &cells[index];
#if LINE_NO_PASS
          RecordMem(get_cur_tid(), &(cnumPars[index]), READ, 814);
#else
          RecordMem(get_cur_tid(), &(cnumPars[index]), READ);
#endif
          int np = cnumPars[index];
          for (int j = 0; j < np; ++j) {
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->density[j % PARTICLES_PER_CELL]),
            // WRITE);
            cell->density[j % PARTICLES_PER_CELL] = 0.0;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->a[j % PARTICLES_PER_CELL]), WRITE);
...
public:
  ComputeDensitiesMTWorker(int id, int tid_, int s, int e) : pid(id), tid(tid_), start(s), end(e) {}
    ...
    for (int iz = sz; iz < ez; ++iz) {
      ...
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        ...
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          int index = (iz * ny + iy) * nx + ix;
#if LINE_NO_PASS
          RecordMem(get_cur_tid(), &(cnumPars[index]), READ, 867);
#else
          RecordMem(get_cur_tid(), &(cnumPars[index]), READ);
#endif
          int np = cnumPars[index];
...
#if LINE_NO_PASS
              RecordMem(get_cur_tid(), &(cnumPars[indexNeigh]), READ, 880);
#else
              RecordMem(get_cur_tid(), &(cnumPars[indexNeigh]), READ);
#endif
...
//              shivam++;
              ...
              for (int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh) {
                ...
                if (&neigh->p[iparNeigh % PARTICLES_PER_CELL] <
                    &cell->p[ipar % PARTICLES_PER_CELL]) {
                  // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[ipar % PARTICLES_PER_CELL]),
                  // READ); AV_Detector::RecordMem(get_cur_tid(), &(neigh->p[iparNeigh %
                  // PARTICLES_PER_CELL]), READ);
                  ...
                  if (distSq < hSq) {
                    ...
                    // AV_Detector::RecordMem(get_cur_tid(), &(border[index]), READ);
                    if (border[index]) {
                      tbbMutex::scoped_lock lock(mutex[index][ipar % MUTEXES_PER_CELL]);
                      // RecordMem(get_cur_tid(), &(cell->density[ipar % PARTICLES_PER_CELL]),
                      // WRITE);
                    ...
                    if (border[indexNeigh]) {
                      ...
                      // RecordMem(get_cur_tid(), &(neigh->density[iparNeigh % PARTICLES_PER_CELL]),
                      // WRITE);
      ...
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        ...
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          ...
          for (int j = 0; j < np; ++j) {
            ...
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->density[j % PARTICLES_PER_CELL]),
            // WRITE); AV_Detector::RecordMem(get_cur_tid(), &(densityCoeff), WRITE);
...
public:
  ComputeForcesMTWorker(int id, int tid_, int s, int e) : pid(id), tid(tid_), start(s), end(e) {}
              ...
              for (int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh) {
                ...
                if (&neigh->p[iparNeigh % PARTICLES_PER_CELL] <
                  ...
                  if (distSq < hSq) {
                    ...
                    // AV_Detector::RecordMem(get_cur_tid(), &(pressureCoeff), READ);
                    // AV_Detector::RecordMem(get_cur_tid(), &(cell->density[ipar %
                    // PARTICLES_PER_CELL]), READ); AV_Detector::RecordMem(get_cur_tid(),
                    // &(neigh->density[iparNeigh % PARTICLES_PER_CELL]), READ);
                    ...
                    // AV_Detector::RecordMem(get_cur_tid(), &(neigh->v[iparNeigh %
                    // PARTICLES_PER_CELL]), READ); AV_Detector::RecordMem(get_cur_tid(),
                    // &(cell->v[ipar % PARTICLES_PER_CELL]), READ);
                    // AV_Detector::RecordMem(get_cur_tid(), &(viscosityCoeff), READ);
                    acc += (neigh->v[iparNeigh % PARTICLES_PER_CELL] -
                            cell->v[ipar % PARTICLES_PER_CELL]) *
                           viscosityCoeff * hmr;
                    // AV_Detector::RecordMem(get_cur_tid(), &(cell->density[ipar %
                    // PARTICLES_PER_CELL]), READ); AV_Detector::RecordMem(get_cur_tid(),
                    // &(neigh->v[iparNeigh % PARTICLES_PER_CELL]), READ);
                    ...
                    if (border[index]) {
                      tbbMutex::scoped_lock lock(mutex[index][ipar % MUTEXES_PER_CELL]);
                      // RecordMem(get_cur_tid(), &(cell->a[ipar % PARTICLES_PER_CELL]), WRITE);
                    ...
                    if (border[indexNeigh]) {
                      tbbMutex::scoped_lock lock(mutex[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
                      // RecordMem(get_cur_tid(), &(neigh->a[iparNeigh % PARTICLES_PER_CELL]),
                      // WRITE);
...
#else
    // AV_Detector::RecordMem(get_cur_tid(), &(grids[tid].sz), READ);
    // AV_Detector::RecordMem(get_cur_tid(), &(grids[tid].ez), READ);
      ...
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        ...
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          ...
          for (int j = 0; j < np; ++j) {
            ...
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[ji]), READ);
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji]), READ);
            Vec3 pos = cell->p[ji] + cell->hv[ji] * timeStep;

            if (ix == 0) {
              fptype diff = parSize - (pos.x - domainMin.x);
              if (diff > epsilon) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].x), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->a[ji].x), WRITE);
            ...
            if (ix == (nx - 1)) {
              ...
              if (diff > epsilon) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].x), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->a[ji].x), WRITE);
                cell->a[ji].x -= stiffnessCollisions * diff + damping * cell->v[ji].x;
              }
            }
            if (iy == 0) {
              ...
              if (diff > epsilon) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].y), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->a[ji].y), WRITE);
            ...
            if (iz == 0) {
              ...
              if (diff > epsilon) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].z), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->a[ji].z), WRITE);
		...
		if(!((ix==0)||(iy==0)||(iz==0)||(ix==(nx-1))||(iy==(ny-1))==(iz==(nz-1))))
          ...
          for (int j = 0; j < np; ++j) {
            ...
            if (ix == 0) {
              ...
              if (diff < Zero) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[ji].x), WRITE);
                cell->p[ji].x = domainMin.x - diff;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].x), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].x), WRITE);
                cell->v[ji].x = -cell->v[ji].x;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].x), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].x), WRITE);
            ...
            if (iy == 0) {
              ...
              if (diff < Zero) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[ji].y), WRITE);
                cell->p[ji].y = domainMin.y - diff;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].y), READ);
                /// AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].y), WRITE);
                cell->v[ji].y = -cell->v[ji].y;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].y), READ);
                /// AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].y), WRITE);
            ...
            if (iz == 0) {
              ...
              if (diff < Zero) {
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[ji].z), WRITE);
                cell->p[ji].z = domainMin.z - diff;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].z), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[ji].z), WRITE);
                cell->v[ji].z = -cell->v[ji].z;
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].z), READ);
                // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[ji].z), WRITE);
...
public:
  AdvanceParticlesMTWorker(int id, int tid_, int s, int e) : pid(id), tid(tid_), start(s), end(e) {}
    ...
    for (int iz = sz; iz < ez; ++iz) {
      ...
      for (int iy = grids[tid].sy; iy < grids[tid].ey; ++iy) {
        ...
        for (int ix = grids[tid].sx; ix < grids[tid].ex; ++ix) {
          ...
          int np = cnumPars[index];
          for (int j = 0; j < np; ++j) {
            // RecordMem(get_cur_tid(), &(cell->hv[j % PARTICLES_PER_CELL]), READ);
            // RecordMem(get_cur_tid(), &(cell->a[j % PARTICLES_PER_CELL]), READ);
            Vec3 v_half =
                cell->hv[j % PARTICLES_PER_CELL] + cell->a[j % PARTICLES_PER_CELL] * timeStep;
#if defined(USE_ImpeneratableWall)
            ...
            // beyond domain
#endif
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->p[j % PARTICLES_PER_CELL]), WRITE);
            cell->p[j % PARTICLES_PER_CELL] += v_half * timeStep;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[j % PARTICLES_PER_CELL]), READ);
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[j % PARTICLES_PER_CELL]), WRITE);
            cell->v[j % PARTICLES_PER_CELL] = cell->hv[j % PARTICLES_PER_CELL] + v_half;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->v[j % PARTICLES_PER_CELL]), WRITE);
            cell->v[j % PARTICLES_PER_CELL] *= 0.5;
            // AV_Detector::RecordMem(get_cur_tid(), &(cell->hv[j % PARTICLES_PER_CELL]), WRITE);
...
//  std::cout << "Num_active_tasks before ClearParticlesMTWorker " << num_active_tasks << "\n";
//  std::cout << "shivam " << shivam << "\n";
  GridLauncher<ClearParticlesMTWorker> &cp =
      *new (tbb::task::allocate_root()) GridLauncher<ClearParticlesMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(cp);
//  std::cout << "shivam " << shivam << "\n";
//
//  std::cout << "Num_active_tasks after ClearParticlesMTWorker " << num_active_tasks << "\n";

  GridLauncher<RebuildGridMTWorker> &rg =
      *new (tbb::task::allocate_root()) GridLauncher<RebuildGridMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(rg);

  GridLauncher<InitDensitiesAndForcesMTWorker> &id =
      *new (tbb::task::allocate_root()) GridLauncher<InitDensitiesAndForcesMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(id);

//  std::cout << "Num_active_tasks before ComputeDensitiesMTWorker " << num_active_tasks << "\n";
//  std::cout << "shivam " << shivam << "\n";
  GridLauncher<ComputeDensitiesMTWorker> &cd =
      *new (tbb::task::allocate_root()) GridLauncher<ComputeDensitiesMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(cd);
//  std::cout << "shivam " << shivam << "\n";

//  std::cout << "Num_active_tasks after ComputeDensitiesMTWorker " << num_active_tasks << "\n";
  GridLauncher<ComputeDensities2MTWorker> &cd2 =
      *new (tbb::task::allocate_root()) GridLauncher<ComputeDensities2MTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(cd2);

  GridLauncher<ComputeForcesMTWorker> &cf =
      *new (tbb::task::allocate_root()) GridLauncher<ComputeForcesMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(cf);

  GridLauncher<ProcessCollisionsMTWorker> &pc =
      *new (tbb::task::allocate_root()) GridLauncher<ProcessCollisionsMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(pc);

  GridLauncher<AdvanceParticlesMTWorker> &ap =
      *new (tbb::task::allocate_root()) GridLauncher<AdvanceParticlesMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(ap);

#if defined(USE_ImpeneratableWall)
  // N.B. The integration of the position can place the particle
  // outside the domain. We now make a pass on the perimiter cells
  // to account for particle migration beyond domain.
  GridLauncher<ProcessCollisions2MTWorker> &pc2 =
      *new (tbb::task::allocate_root()) GridLauncher<ProcessCollisions2MTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(pc2);
#endif
}
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  if (argc < 4 || argc >= 6) {
    std::cout << "Usage: " << argv[0]
              << " <threadnum> <framenum> <.fluid input file> [.fluid output file]" << std::endl;
    return -1;
  }
...
#ifdef ENABLE_CFL_CHECK
  std::cout << "WARNING: Check for Courant–Friedrichs–Lewy condition enabled. Do not use for "
