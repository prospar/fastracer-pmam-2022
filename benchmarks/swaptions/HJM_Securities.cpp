...
//   std::cout << "HJM_Securities operator taskid " << getTaskId() << "\n";
//    set_complete();

    FTYPE CHECK_AV pdSwaptionPrice[2];
    int begin = range.begin();
    int end = range.end();

    for (int i = begin; i != end; i++) {
      // RecordMem(thdId, pdSwaptionPrice, READ);
      // RecordMem(thdId, &(swaptions[i].dStrike), READ);
      // RecordMem(thdId, &(swaptions[i].dCompounding), READ);
      // RecordMem(thdId, &(swaptions[i].dMaturity), READ);
      // RecordMem(thdId, &(swaptions[i].dTenor), READ);
      // RecordMem(thdId, &(swaptions[i].dPaymentInterval), READ);
      // RecordMem(thdId, &(swaptions[i].iN), READ);
      // RecordMem(thdId, &(swaptions[i].iFactors), READ);
      // RecordMem(thdId, &(swaptions[i].dYears), READ);
      int iSuccess = HJM_Swaption_Blocking(
          pdSwaptionPrice, swaptions[i].dStrike, swaptions[i].dCompounding, swaptions[i].dMaturity,
          swaptions[i].dTenor, swaptions[i].dPaymentInterval, swaptions[i].iN,
          swaptions[i].iFactors, swaptions[i].dYears, swaptions[i].pdYield, swaptions[i].ppdFactors,
          swaption_seed + i, NUM_TRIALS, BLOCK_SIZE, 0);
//      int iSuccess = 1;
      assert(iSuccess == 1);
      RecordMem(thdId, &(pdSwaptionPrice[0]), READ);
      RecordMem(thdId, &(swaptions[i].dSimSwaptionMeanPrice), WRITE);
      swaptions[i].dSimSwaptionMeanPrice = pdSwaptionPrice[0];
      RecordMem(thdId, &(pdSwaptionPrice[1]), READ);
...
void *worker(void *arg) {
  ...
  int CHECK_AV beg, CHECK_AV end, CHECK_AV chunksize;
  ...
  for (int i = beg; i < end; i++) {
    int iSuccess = HJM_Swaption_Blocking(
        pdSwaptionPrice, swaptions[i].dStrike, swaptions[i].dCompounding, swaptions[i].dMaturity,
        swaptions[i].dTenor, swaptions[i].dPaymentInterval, swaptions[i].iN, swaptions[i].iFactors,
        swaptions[i].dYears, swaptions[i].pdYield, swaptions[i].ppdFactors, swaption_seed + i,
...
// Please note: Whenever we type-cast to (int), we add 0.5 to ensure that the value is rounded to
// the correct number. For instance, if X/Y = 0.999 then (int) (X/Y) will equal 0 and not 1 (as
// (int) rounds down). Adding 0.5 ensures that this does not happen. Therefore we use (int) (X/Y +
// 0.5); instead of (int) (X/Y);
