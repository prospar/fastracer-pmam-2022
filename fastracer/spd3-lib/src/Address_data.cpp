...
void Address_data::printAddressData() {
  ...
  for (std::vector<struct Lock_version*>::iterator it1 = lockset.begin(); it1 != lockset.end();
       ++it1) {
...
bool Address_data::locksetEmpty(std::vector<struct Lock_version*> l1,
...
bool Address_data::conflictingAccess(struct Address_data* a1, struct Address_data* a2) {
  return (a1->task->taskId != a2->task->taskId &&
          (a1->accessType == WRITE || a2->accessType == WRITE));
}
