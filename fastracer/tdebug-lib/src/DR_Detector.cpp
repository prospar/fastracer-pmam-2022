...
unsigned recordmemt = 0, recordmemn = 0, recordaccesst = 0, recordaccessn = 0, spawnt = 0,
      ...
      if ((f_entry.lockset != 0) && (((~curLockset) & (~f_entry.lockset)) == 0)) {
        ...
        if (accessType == WRITE) {
          if (f_entry.r1_task != NULL &&
            ...
            all_violations.insert(std::pair<ADDRINT, struct violation*>(
                addr,
                new violation(new violation_data(curStepNode, accessType, line_no),
          ...
          if (((~curLockset) & (~add_data.lockset)) == 0) {
            ...
            if (accessType == WRITE) {
              if (add_data.r1_task != NULL && taskGraph->areParallel(tidToTaskIdMap[threadid].top(),
                ...
                all_violations.insert(std::pair<ADDRINT, struct violation*>(
                    addr, new violation(
                              new violation_data(curStepNode, accessType, line_no),
