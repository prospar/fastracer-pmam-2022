...
template <class T> class ParticleFilterTBB : public ParticleFilter<T>, public tbb::filter {
...
protected:
  ...
  void CalcWeights(
      std::vector<Vectorf> &particles); // calculate particle weights based on model likelihood
...
protected:
  ...
  template <class S> class DoGenerateNewParticlesTBB {
    ...
    inline void AddGaussianNoise(Vectorf &p, const Vectorf &stdDevs, RandomGenerator &rnd,
                                 size_t thdId) const {
  ...
  public:
    void operator()(const tbb::blocked_range<int> &r, size_t thdId) const {
      for (int i = r.begin(); i < r.end(); i++) {
        // RecordMem(thdId, &mIndex[i], READ);
        RecordMem(thdId, &mParticles[mIndex[i]], READ);
        mNewParticles[i] = mParticles[mIndex[i]];
        AddGaussianNoise(mNewParticles[i], mModel->StdDevs()[k], mRnd[i], thdId);
      }
    }
    DoGenerateNewParticlesTBB(int _k, S *_model, std::vector<Vectorf> &_mNewParticles,
                              std::vector<Vectorf> &_mParticles,
                              std::vector<RandomGenerator> &_mRnd, std::vector<int> &_mIndex)
        : mModel(_model), mNewParticles(_mNewParticles), mParticles(_mParticles), mRnd(_mRnd),
          mIndex(_mIndex), k(_k) {}
  };

  // likelihood block computing object
  template <class S> class DoCalcLikelihoods {
  private:
    S *mModel;
    Vectorf *mParticles;
    fpType *mWeights;
  ...
  public:
    DoCalcLikelihoods(S *model, Vectorf *particles, fpType *weights, unsigned char *valid)
        : mModel(model), mParticles(particles), mWeights(weights), mValid(valid){};

    void operator()(const tbb::blocked_range<int> &r, size_t thdid) const {
      for (int i = r.begin(); i != r.end(); ++i) {
        bool vflag;
        mWeights[i] = mModel->LogLikelihood(mParticles[i], vflag,
                                            i); // compute log-likelihood weights for each particle
        mValid[i] = vflag ? 1 : 0;
      }
    }
  };
...
template <class T> void ParticleFilterTBB<T>::CalcWeights(std::vector<Vectorf> &particles) {
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, np, WORKUNIT_SIZE_CALCWEIGHTS),
  ...
  if ((int)particles.size() < mMinParticles)
    return;
  mWeights -= minWeight; // shift weights to zero for numerical stability
  if (mModel->StdDevs().size() > 1)
    annealingFactor =
        BetaAnnealingFactor(mWeights, 0.5f); // calculate annealing factor if more than 1 step
...
template <class T> void ParticleFilterTBB<T>::GenerateNewParticles(int k) {
  int p = 0;
  ...
  // TBB parallel_for
  parallel_for(
      tbb::blocked_range<int>(0, mNParticles, WORKUNIT_SIZE_NEWPARTICLES),
...
template <class T>
bool ParticleFilterTBB<T>::Update(
    fpType timeval) // weights have already been computed from previous step or initialization
{
  ...
  for (int k = (int)mModel->StdDevs().size() - 1; k >= 0;
       k--) // loop over all annealing steps starting with highest
  {
...
template <class T> void ParticleFilterTBB<T>::WritePose(std::ostream &f, std::vector<float> &pose) {
  for (int i = 0; i < (int)pose.size(); i++)
    f << pose[i] << " ";
  f << std::endl;
}

template <class T> void *ParticleFilterTBB<T>::operator()(void *token) {
