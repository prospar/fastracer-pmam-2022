...
public:
	void operator()(const blocked_range<int>& range, size_t thdid) const
	{
		...
		for(int y = range.begin(); y < range.end(); y++)
		{
			...
			{
			  //RecordMem(thdid, &ph[-1], READ);
			  //RecordMem(thdid, &ph[1], READ);
			  //RecordMem(thdid, &p[-1], READ);
			  //RecordMem(thdid, &p[1], READ);
			  //RecordMem(thdid, &pl[-1], READ);
			  RecordMem(thdid, &pl[1], READ);
			  float xg = -0.125f * ph[-1] + 0.125f * ph[1] - 0.250f * p[-1] + 0.250f * p[1] - 0.125f * pl[-1] + 0.125f * pl[1];	//calc x and y gradients
			  //RecordMem(thdid, &ph[-1], READ);
			  //RecordMem(thdid, &ph[0], READ);
			  //RecordMem(thdid, &ph[1], READ);
			  //RecordMem(thdid, &pl[0], READ);
			  //RecordMem(thdid, &pl[-1], READ);
			  //RecordMem(thdid, &pl[1], READ);
...
template<class T, class T2> 
  ...
  public:
    ...
    void operator() ( const blocked_range<int>& r, size_t thdid) const{
	  ...
	  for(int x = n; x < source_width - n; x++)
	    {
	      ...
	      T2 acc = 0; 
	      for(int i = -n; i <= n; i++){
		//RecordMem(thdid, &psrc[i], READ);
		//RecordMem(thdid, &kernel[k], READ);
    ...
    void operator() ( const blocked_range<int>& r, size_t thdid) const
	{
		...
		for(int y = r.begin(); y < r.end(); y++)
		{
		  ...
		  for(int x = 0; x < source_width; x++)
		  {
			...
			T2 acc = 0; 
			for(int i = -n; i <= n; i++) {
			  //RecordMem(thdid, &(*(T *)((char *)psrc + sb * i)), READ);
			  //RecordMem(thdid, &kernel[k], READ);
