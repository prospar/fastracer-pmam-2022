...
public:
  ...
  void dump() {
    ...
    std::string bench;
    std::ifstream("/proc/self/comm") >> bench;
    //std::cout << "PATH==" << sp <<"\n";
    std::string csv_path = getenv("LD_LIBRARY_PATH");
    csv_path.append("/../../stats_dump.csv");
    //std::cout << "PATH==" << csv_path <<"\n";

    std::ofstream fin;
    // Open an existing file
    fin.open(csv_path, std::ios::out | std::ios::app);
    ...
    if (fin.tellp() == 0) {
      ...
      fin << "Total reads"
          ...
          << "Max deoth of task"
          << ", "
          << "Max rdr tasks across all var"
          << ", "
          << "Max wrtr tasks across all var"
          << ", ";
      fin << "Max reads across all var"
          << ", "
          << "Max writes across all var"
          << ", "
          << "Max rds by a task"
          << ", "
          << "Max wrts by a task"
          << ", ";
      fin << "Min rds by a task"
          << ", "
          << "Min wrts by a task"
          << ", "
          << "Max num of vars read by a task"
          << ", "
          << "Max num of vars wrt by a task"
          << ", ";
      fin << "Max locks acq by a task"
          << ", "
          << "Max locks rel by a task"
          << "\n";
    }

    fin << "PTRacer"
        << ", " << bench << ", " << total_num_viol << ", " << task_id_ctr << ", "
        << total_num_recordmems << ", " << total_num_recordaccess << ", ";
    fin << total_num_rds << ", " << total_num_wrs << ", " << max_task_depth << ", "
        << pv_max_rds_tasks << ", " << pv_max_wrs_tasks << ", ";
    fin << pv_max_rds << ", " << pv_max_wrs << ", " << pt_max_rds << ", " << pt_max_wrs << ", ";
    fin << pt_min_rds << ", " << pt_min_wrs << ", " << pt_max_vars_read << ", "
        << pt_max_vars_written << ", ";
    fin << pt_max_acqs << ", " << pt_max_rels << "\n";
    fin.close();
