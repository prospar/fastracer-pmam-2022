...
int HJM_SimPath_Yield(FTYPE **ppdHJMPath, int iN, int iFactors, FTYPE dYears, FTYPE *pdYield,
                      FTYPE **ppdFactors, long *lRndSeed);
int HJM_SimPath_Forward(FTYPE **ppdHJMPath, int iN, int iFactors, FTYPE dYears, FTYPE *pdForward,
                        FTYPE *pdTotalDrift, FTYPE **ppdFactors, long *lRndSeed);
...
// int Discount_Factors_early_exit(FTYPE *pdDiscountFactors, int iN, FTYPE dYears, FTYPE
// *pdRatePath, int iSwapStartTimeIndex);
...
int HJM_SimPath_Yield(FTYPE **ppdHJMPath, // Matrix that stores generated HJM path (Output)
  ...
  // generating HJM Path
  iSuccess = HJM_SimPath_Forward(ppdHJMPath, iN, iFactors, dYears, pdForward, pdTotalDrift,
                                 ppdFactors, lRndSeed);
  if (iSuccess != 1) {
    free_dvector(pdForward, 0, iN - 1);
    free_dmatrix(ppdDrifts, 0, iFactors - 1, 0, iN - 2);
...
int HJM_Factors(
    FTYPE **ppdFactors, // Output matrix that stores factor volatilities for different maturities
    int iN, int iFactors,
    ...
    FTYPE **ppdFacBreak) // Input matrix of factor weights for each maturity
{
  ...
  // For instance, the user may supply				   Maturity:	1	   2	  3
  // 4 total vol (pdVol) as
  // Sigma:  1.35%, 1.30%, 1.25%, 1.20%,.... and the weight breakdown (ppdFacBreak) as        Factor
  // 1:   0.55,  0.60,  0.65,  0.69,.... 												   Factor 2:   0.44,  0.39,  0.34,  0.30,.... 												   Factor 3:   0.01,
  //0.01,  0.01,  0.01,.... Note that the weights add up to 1 in each case. Also, the weights are
  // based on variance not volatility.

  // Based on these inputs, the function will calculate individual volatilties for each factor for
  // each maturity. The output (ppdFactors) may look something like: Maturity:	1	   2	  3
  // 4 												   Factor 1:  1.00%  1.00%  1.00%  1.00% 												   Factor 2:  0.90%  0.82%  0.74%  0.67% 												   Factor 3:  0.10%
  //0.08%  0.05%  0.03%
  // (Please note that in this example the figures have been rounded and therefore may not be
  // exact.)
...
{
  // This function computes drift corrections required for each factor for each maturity based on
  // given factor volatilities
    ...
    long *lRndSeed)      // Random number seed
{
  ...
  for (j = 1; j <= iN - 1; ++j) {
    ...
    for (l = 0; l <= iFactors - 1; ++l)
      pdZ[l] =
          CumNormalInv(RanUnif(lRndSeed)); // shocks to hit various factors for forward curve at t
    ...
    for (l = 0; l <= iN - (j + 1); ++l) {
      ...
      ppdHJMPath[j][l] =
          ppdHJMPath[j - 1][l + 1] + pdTotalDrift[l] * ddelt + sqrt(ddelt) * dTotalShock;
      // as per formula
    }
  }
...
int HJM_Correlations(FTYPE **ppdHJMCorr, // Matrix that stores correlations among factor
                                         // volatilities for different maturities
                     int iN, int iFactors, FTYPE **ppdFactors) {
  ...
  FTYPE **ppdWeights; // matrix that stores ratio of each factor to total volatility for different
                      // maturities
...
// ***********************************************************************
// ***********************************************************************
int Discount_Factors_Blocking(FTYPE *pdDiscountFactors, int iN, FTYPE dYears, FTYPE *pdRatePath,
                              int BLOCKSIZE) {
